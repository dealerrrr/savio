#!/usr/bin/env node
// Regenera lib/guardian-corpus.js a partir del documento 12 (base de
// conocimiento del Guardián). El doc 12 es la única fuente de verdad del
// corpus: NUNCA editar lib/guardian-corpus.js a mano.
//
// Uso:
//   node scripts/build-corpus.js            (usa documentos/12_Base_Conocimiento_Guardian.md)
...
//
// Qué hace:
//   1. Lee el doc 12 y valida su estructura (encabezados esperados).
...
//
// El script es determinista: mismo doc 12 → mismo corpus generado.
...
const FUENTE_DEFAULT = path.join(RAIZ, 'documentos', '12_Base_Conocimiento_Guardian.md');
// Copia de trabajo en Obsidian; debe estar sincronizada con documentos/.
const FUENTE_NOTAS =
  '/home/dealerrr/Documentos/Notas/Masoneria/Proyectos/lasavio.com.ar/12_Base_Conocimiento_Guardian.md';
...
const FRASES_META = ['capa de RAG', 'quien implemente el RAG'];
...
// Guarda anti-divergencia: si existen las dos copias del doc 12 (repo y
...
if (fuente === FUENTE_DEFAULT && fs.existsSync(FUENTE_NOTAS)) {
  ...
  if (notas !== md) {
    fallar(
      'Las dos copias del doc 12 difieren.\n' +
        `  repo:    ${FUENTE_DEFAULT}\n` +
...
// Validación de estructura: el doc 12 debe tener estos encabezados.
const ENCABEZADOS_REQUERIDOS = [
  '## Anexo: Base de conocimiento del Guardián',
  '### Cómo se usa este corpus',
  '### Masonería universal: qué es y cómo funciona',
  '### Lista de control',
];
for (const encabezado of ENCABEZADOS_REQUERIDOS) {
  if (!md.includes(encabezado)) {
    fallar(`El doc fuente no parece ser el doc 12: falta el encabezado «${encabezado}».`);
  }
}

// Recorte: el corpus va desde "### Masonería universal..." hasta
// "### Lista de control" (exclusive). Todo lo anterior es preámbulo +
// instrucciones de uso.
const inicio = md.indexOf('### Masonería universal: qué es y cómo funciona');
...
for (const frase of FRASES_META) {
  if (corpus.includes(frase)) {
    fallar(`Se filtró texto meta al corpus: «${frase}». Revisar la estructura del doc 12.`);
  }
}
...
// documento 12 (base de conocimiento aprobada por la logia).
// Para actualizarlo: editar el doc 12 y correr \`node scripts/build-corpus.js\`.
// Instructivo completo: HERRAMIENTAS.md, sección "Actualizar corpus del Guardián".
//
// Trazabilidad:
//   Fuente:    documentos/12_Base_Conocimiento_Guardian.md
//   Generado:  ${fecha}
