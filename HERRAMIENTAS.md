...
| Script que genere variantes responsive (`-480w`/`-800w`) de una imagen `.webp` con `cwebp -resize` y arme el `srcset`/`sizes` para pegar en el HTML | 🟡 candidato | Hecho a mano en la sesión 2026-07-10 para `teatro.webp` y `templo_fachada.webp` (PSI marcaba imágenes sobredimensionadas para mobile). Determinista: mismo archivo + anchos → mismas variantes. Conviene versionarlo si se repite para más imágenes del sitio; con solo 2 casos hasta ahora no es urgente. |
